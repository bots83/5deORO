"""Predicción FINAL con la mejor config encontrada (iter 30).

Probamos las top configs y elegimos la que da mejor balance.
Mejor candidata: ts=150 w=6.0 L=30 n=200 -> top-45 con 5/5 = 42/50 (84%) ✅
"""
import sys
import json
...
# La mejor config para top-45 con 5/5 hits
BEST_CFG = {
    "num_leaves": 30, "max_depth": 5, "learning_rate": 0.03,
    "n_estimators": 200, "reg_alpha": 0.2, "reg_lambda": 0.2,
}
TRAIN_START = 150
WEIGHT_POS = 6.0
...
def main():
    ...
    print(f"\nBacktest con BEST CFG: ts={TRAIN_START}, w_pos={WEIGHT_POS}, leaves={BEST_CFG['num_leaves']}, n_est={BEST_CFG['n_estimators']}", flush=True)
    ...
    for k in [10, 15, 20, 25, 30, 31, 35, 38, 40, 41, 42, 44, 45, 46, 47]:
    ...
    # Re-entrenar con TODO incluyendo últimos 50
    print("\n\n=== Re-entrenando con TODO el histórico para predecir ===", flush=True)
    X_all, y_all, _ = build_meta_dataset(df, TRAIN_START, n_total)
    ...
    output = {
        ...
        "model": "LightGBM Optimal v2 (iter 30 BEST)",
    ...
    levels = [
        ...
        (30, "🏆 META 3+", f"backtest {n_3plus[30]}/50 con ≥3 hits"),
        (35, "ULTRA 3+", f"backtest {n_3plus[35]}/50 con ≥3 hits"),
    ...
    with open("reports/prediction_iter30_best.json", "w") as f:
        json.dump(output, f, indent=2, default=str)
    print(f"\n✓ Guardado en reports/prediction_iter30_best.json")
