"""Predicción con LightGBM weighted (iter 21) - el nuevo GANADOR.

Config: num_leaves=50, max_depth=5, lr=0.03, n_est=250, reg_alpha=0.1, reg_lambda=0.1
Sample weights: positivos=3.0, negativos=1.0

Backtest 50 sorteos:
- Top-31 con ≥3 hits: 38/50 (76%) ✅
- Top-35 con ≥3 hits: 46/50 (92%) ✅
- Top-40 con ≥4 hits: 41/50 (82%) ✅
"""
import sys
import json
...
# Mejor config encontrada en iter 21
BEST_CFG = {
    "num_leaves": 50,
    "max_depth": 5,
    "learning_rate": 0.03,
...
SAMPLE_WEIGHT_POS = 3.0  # de iter 21


def evaluate_full(df, model, feature_names, last_n=50):
    n_total = len(df)
...
def main():
    ...
    n_total = len(df)
    print("\nBuilding meta dataset (todos menos últimos 50 para backtest)...")
    X_train, y_train, feature_names = build_meta_dataset(df, 60, n_total - 50)

    sample_weight = np.where(y_train == 1, SAMPLE_WEIGHT_POS, 1.0)
    print(f"Train: {X_train.shape}, sample_weight pos={SAMPLE_WEIGHT_POS}")
    ...
    n_5of5, n_4plus, n_3plus, n_2plus, n_1plus = evaluate_full(df, model, feature_names, last_n=50)

    print(f"\n{'Top-K':<8} {'5/5':>5} {'4+':>5} {'3+':>5} {'2+':>5} {'1+':>5}")
    for k in [10, 12, 15, 18, 20, 25, 30, 31, 35, 38, 40, 45]:
    ...
    # Re-entrenar con TODO incluyendo últimos 50, para el próximo
    print("\n\n=== Re-entrenando con TODO el histórico ===")
    X_all, y_all, _ = build_meta_dataset(df, 60, n_total)
    sw_all = np.where(y_all == 1, SAMPLE_WEIGHT_POS, 1.0)
    final_model = LGBMRegressor(**BEST_CFG, verbose=-1)
    final_model.fit(X_all, y_all, sample_weight=sw_all)
    print(f"Re-entrenado con {X_all.shape[0]} muestras")

    # Predecir próximo sorteo
    print("\n" + "=" * 80)
    print("🎯 PREDICCIÓN PARA EL PRÓXIMO SORTEO (LightGBM Weighted)")
    print("=" * 80)
    ...
    output = {
        ...
        "model": "LightGBM Weighted (iter 21)",
        "model_config": BEST_CFG,
        "sample_weight_pos": SAMPLE_WEIGHT_POS,
        "predictions": {},
        "backtest_50": {
    ...
    levels = [
        ...
        (35, "ULTRA", f"backtest {n_3plus[35]}/50 con ≥3 hits"),
        (40, "🏆 META 4+", f"backtest {n_4plus[40]}/50 con ≥4 hits"),
    ]
    ...
    with open("reports/prediction_weighted.json", "w") as f:
        json.dump(output, f, indent=2, default=str)
    print(f"\n✓ Guardado en reports/prediction_weighted.json")
